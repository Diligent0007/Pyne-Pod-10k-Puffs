...
-- Create window (no built-in toggle key)
local win = lib:Window("Diligence | Bit Of Everything | V67 | Diligent.gbp ", Color3.fromRGB(44, 120, 224))

-- ==============================
-- Saved Toggle Key
-- ==============================
local saveFile = "Diligence_ToggleKey.txt"
local toggleKey = Enum.KeyCode.RightShift -- fallback default
...
-- Listen for toggle key presses
game:GetService("UserInputService").InputBegan:Connect(function(input, gpe)
    if gpe then return end
    if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == toggleKey then
        guiVisible = not guiVisible
        win:SetVisible(guiVisible)
    end
end)
